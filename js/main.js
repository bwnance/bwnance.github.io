...
(function($) {
  ...
  var ssMasonryFolio = function() {
    ...
    containerBricks.imagesLoaded(function() {
      containerBricks.masonry({
        ...
        resize: true,
        // columnWidth: 500,
        // columnWidth: ".masonry-wrap",
        // percentPosition: true,
        // fitWidth: true,
        gutter: 50
      });
      $(".item-folio__project-link").click(e => {
        window.open(e.currentTarget.attributes[0].value)
      });
    });
  };
  ...
  var ssPhotoswipe = function() {
    ...
    $folioItems.each(function(i) {
      var $folio = $(this),
        $thumbLinks = $folio.find(".thumb-link"),
        $title = $folio.find(".item-folio__title"),
        $caption = $folio.find(".item-folio__caption"),
        $titleText = "<h4>" + $.trim($title.html()) + "</h4>",
        $captionText = $.trim($caption.html()),
        // $hrefs = $thumbLink.attr("href"),
        $size = $thumbLinks.data("size").split("x"),
        $width = $size[0],
        $height = $size[1];
      const thumb_items = []
      // debugger
      $thumbLinks.each(index => {
        thumb_items.push({
          src: $($thumbLinks[index]).attr("href"),
          w: $width,
          h: $height
        });
      });
      ...
      // }
      const $folioItem = $(".item-folio")[i];
      items[$folioItem.id] = thumb_items;
    });
